...
export default function EditAnnouncement() {
  ...
  const fetchAnnouncement = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/announcements/${id}`, getAuthHeader());
      ...
      if (res.data.success) {
        ...
        setFormData({
          ...
          type: ann.type || "Offer / Promotion",
          targetAudience: ann.targetAudience?.[0] || "All Users",
          publishDate: ann.publishDate ? ann.publishDate.split("T")[0] : "",
          expiryDate: ann.expiryDate ? ann.expiryDate.split("T")[0] : "",
          isPinned: ann.priorityVisibility?.isPinned || false,
          sendEmailNotification: ann.priorityVisibility?.sendEmailNotification || false,
          showHomepageBanner: ann.priorityVisibility?.showHomepageBanner || false,
          isSpecialOffer: ann.isSpecialOffer || false,
          couponCode: ann.couponCode || "",
          saveAsDraft: ann.status === "Draft",
          publishNow: ann.status === "Published",
        });
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await axios.patch(`${API_URL}/announcements/admin/${id}`, payload, getAuthHeader());
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F7F9FB] font-sans flex items-center justify-center">
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Edit Announcement</h1>
              <p className="text-gray-500 mt-1">
                Update and manage your announcement details.
              </p>
            </div>
            <Link
          ...
          <div className="flex-1 space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Update the announcement message for your users</p>
                  </div>
            ...
            {step === 4 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    <h2 className="text-lg font-bold text-gray-800">Review & Update</h2>
            ...
            <div className="flex items-center justify-between pt-4">
              ...
              {step < 4 ? (
                <button
                  ...
                  className="px-6 py-2.5 bg-emerald-600 text-white rounded-full text-sm font-semibold hover:bg-emerald-700 transition-all"
                >
                  Continue
                </button>
              ) : (
                <div className="flex gap-3">
                  <button
                    ...
                    className="px-6 py-2.5 border-2 border-gray-300 text-gray-600 rounded-full text-sm font-semibold hover:bg-gray-100 transition-all"
                  >
                    Cancel
                  </button>
                  <button
